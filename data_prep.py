...
def x_clean(x_val):
    ...
    x_val['race'] = x_val.race.replace({'AfricanAmerican': 'race_black',
                                        ...
                                        'Hispanic': 'race_other',
                                        'Other': 'race_other'})
    x_val['max_glu_serum'] = x_val.max_glu_serum.replace({'None': 0,
                                                          'Norm': 1,
                                                          '>200': 2,
                                                          '>300': 3})
    x_val['age'] = x_val.age.replace({'[0-10)': 0, '[10-20)': 0,
                                      ...
                                      '[60-70)': 4, '[70-80)': 5,
                                      '[80-90)': 6, '[90-100)': 7})
    x_val['A1Cresult'] = x_val.A1Cresult.replace({'None': 0, 'Norm': 1,
                                                  '>7': 2, ">8": 3})
    ...
    x_val['at_id'] = x_val.at_id.replace({1: 'at_urgent',
                                          2: 'at_urgent',
                                          3: 'at_not_urgent',
                                          5: 'at_no_info',
                                          6: 'at_no_info',
                                          7: 'at_urgent',
                                          8: 'at_no_info'})
    x_val['dd_id'] = x_val['dd_id'].replace({1: 'dd_home',
                                             2: 'dd_care',
                                             3: 'dd_care',
                                             4: 'dd_care',
                                             5: 'dd_care',
...
def diagnoses_1_3(x_val):
    ...
    x_val['icd_2'] = x_val.swifter.apply(lambda row: row.icd_2_1
                                         + row.icd_2_2 + row.icd_2_3,
                                         axis=1)
    ...
    x_val['icd_3'] = x_val.swifter.apply(lambda row: row.icd_3_1
                                         + row.icd_3_2 + row.icd_3_3,
                                         axis=1)
    ...
    x_val['icd_4'] = x_val.swifter.apply(lambda row: row.icd_4_1
                                         + row.icd_4_2 + row.icd_4_3,
                                         axis=1)
    ...
    x_val['icd_5'] = x_val.swifter.apply(lambda row: row.icd_5_1
                                         + row.icd_5_2 + row.icd_5_3,
                                         axis=1)
    ...
    x_val['icd_6'] = x_val.swifter.apply(lambda row: row.icd_6_1
                                         + row.icd_6_2 + row.icd_6_3,
                                         axis=1)
...
def integer(x_val):
    """Returns features in int64 form"""
...
def outliers_bin(x_val):
    ...
    for col in binned_columns:
        x_val[f'{col}_binned'] = x_val[col].swifter.apply(lambda x: 3
                                                          if x > 3 else
                                                          (2 if 2 <= x <= 3
                                                           else x))
...
def clean():
    ...
    x_val, y_val = upload()
    y_val = y_clean(y_val)
    x_val = null_value_drop(x_val)
    x_val = drop_invalid_data(x_val)
    x_val, y_val = reset_indices(x_val, y_val)
    x_val = column_drop(x_val)
    x_val = x_clean(x_val)
    x_val = values_lower(x_val)
    x_val = column_lowercase(x_val)
    x_val = categorize_all(x_val)
    x_val = diagnoses_1_3(x_val)
    x_val = diagnoses_2_3(x_val)
    x_val = diagnoses_3_3(x_val)
    x_val = med_columns(x_val)
    x_val = x_dummy_variables(x_val)
    x_val = integer(x_val)
    x_val = outliers_bin(x_val)
    x_val = outliers_log(x_val)
    return x_val, y_val
